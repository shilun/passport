...
public class OldController extends AbstractClientController {
    ...
    public String reg(String recommendId, Model model) {
        ...
        if (agentType == AgentTypeEnum.Android || agentType == AgentTypeEnum.Other) {
            getRequest().setAttribute("url", softWareService.findLastInfo(getDomain().getId(), AgentTypeEnum.Android).getUrl());
        }
        if (agentType == AgentTypeEnum.IOS) {
            getRequest().setAttribute("url", "itms-services://?action=download-manifest&url=/AppDownload/download.plist");
        }
        return "/register";
    }
